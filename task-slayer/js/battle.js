...
const fightStartButton = document.getElementById("fightStartButton");
const battleScreen = document.querySelector("#battle-scene .battle-screen");
...
function getEquippedByCategory(categoryMap) {
  return getEquippedShopIds().find((id) => Object.prototype.hasOwnProperty.call(categoryMap, id)) ?? null;
}
...
function renderEquippedTitle() {
  ...
  badge.className = "equipped-title";
  badge.textContent = titleText;

  if (headerUsername) {
    headerUsername.insertAdjacentElement("afterend", badge.cloneNode(true));
  }

  const heroPanelTitle = document.querySelector("#info-scene .hero-panel .panel-title-row h2");
  if (heroPanelTitle) {
    heroPanelTitle.appendChild(badge);
  }
}
...
function setBattleHeroState(state) {
  ...
  battleHeroFrame.classList.remove("is-running-forward", "is-attacking", "is-running-back", "is-returned", "is-hit", "faded-out");
  ...
  if (state === "hit") {
    battleHeroFrame.classList.add("is-hit", "is-returned");
    restartGif(battleHeroMedia, assetConfig.heroIdle);
  }
}
...
function renderBattleState() {
  ...
  if (playerCoins) playerCoins.textContent = battleState.coinsInfinite ? "∞" : battleState.coins;
...
function hideFightOverlay(shouldStart = false) {
  if (fightOverlay) {
    fightOverlay.hidden = true;
  }
  fightStartButton?.classList.remove("is-starting");
  battleScreen?.classList.remove("fight-shake");
  if (fightOverlayResolve) {
    fightOverlayResolve(shouldStart);
    fightOverlayResolve = null;
  }
}

function showFightOverlay() {
  if (!fightOverlay || !fightStartButton) {
  ...
  battleStatusText.textContent = "Awaiting your command.";

  return new Promise((resolve) => {
    fightOverlayResolve = resolve;
    const startFight = () => {
      fightStartButton.classList.add("is-starting");
      battleScreen?.classList.add("fight-shake");
      window.setTimeout(() => {
        hideFightOverlay(true);
      }, 220);
    };

    fightStartButton.addEventListener("click", startFight, { once: true });
  });
}
...
async function handleProgressUpdate(event) {
  ...
  let percent = Number(progressRangeInput.value);
  const note = progressNoteInput.value.trim() || "Daily progress update submitted.";

  const currentProgress = Math.max(Number(task.progress) || 0, Number(progressRangeInput.dataset.floor) || 0);

  if (percent <= currentProgress) {
    percent = Math.min(100, currentProgress + 5);
    progressRangeInput.value = String(percent);
    progressRangeValue.textContent = `${percent}%`;
  }

  if (percent <= currentProgress) {
    addLogEntry(combatLog, "This quest is already at maximum progress.");
    return;
  }
  ...
  try {
    ...
    if (data.playerStats.coinsInfinite) {
      battleState.coinsInfinite = true;
    } else if (data.playerStats.coins != null) {
      battleState.coins = data.playerStats.coins;
    }
    ...
    const shouldStartFight = await showFightOverlay();
    if (!shouldStartFight || battleState.currentScene !== "battle") {
...
async function initApp() {
  ...
  try {
    ...
    Object.assign(battleState, {
      ...
      coinsInfinite: Boolean(statsData.coinsInfinite),
